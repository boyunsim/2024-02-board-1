...
/*
    ...
    Annotation(애노테이션)
    @Controller - 응답을 html로 한다 (데이터로 만든 화면을 응답)
    ...
    브라우저의 주소창에 주소값을 적고 엔터 = URL + GET방식 + 데이터 보내는 방식(Key/Value)으로 요청을 보낸다.
    데이터를 보낼 때 보여지나 안 보여지나의 차이가 있음
    ...
    JSON(JavaScript Object Notation): 자바스크립트에서 객체를 만들 때 사용하는 문법을 이용하여
                                      데이터를 표현하는 포멧(형식), Key와 Value로 이루어져 있음.
    ...
    url은 같은데 method로 작업을 구분했다. (마치 if문 처럼)

    위 작업은 두가지 밖에 없었기 때문에 같은 주소값으로 method를 구분할 수 있었다.
    그런데 CRUD(작업이 4가지)를 해야하는 상황에서는 작업 구분을 주소값으로 해야했었다.
    ...
    첫 페이지(index화면)를 띄울 때 화면을 띄울 수 있는 소프트웨어(FE 작업 코드)가 모두 다운로드 됨.
    ...
    GET, DELETE방식은 Querry String or Path Variable을 사용해서 데이터를 보낸다.
    ...
    (delete) /board - 글 삭제 (Path Variable or Querry String으로 pk값 전달)
*/
//final 붙은 멤버필드 DI받을 수 있게 생성자를 만든다.
//애노테이션 생략시 오버로딩된 생성자를 직접 만들어줘야한다.
